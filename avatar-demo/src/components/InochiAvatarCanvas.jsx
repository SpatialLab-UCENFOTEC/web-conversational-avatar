  ...
  const applyParams = () => {
    ...
    try {
      ...
      if (headParamRef.current) {
        ...
        if (stateRef.current.mode === "speaking") {
          const t = performance.now() / 1000;
          targetX += Math.sin(t * 2.0) * 0.01;
        }
      ...
      // Respiración suave si el avatar tiene parámetro compatible
      idleTimeRef.current += 0.016;

      const idleBreath =
        0.5 + Math.sin(idleTimeRef.current * 1.4) * 0.08;
    ...
    setMode(mode) {
      ...
      switch (mode) {
        case "listening":
          ...
          stateRef.current.mouthOpen = 0.65;
          expressionControllerRef.current?.apply("listening");
          break;

        case "speaking":
          targetHeadRef.current = { x: 0.08, y: 0.02 };
          expressionControllerRef.current?.apply("affirmative");
          break;

        default:
          targetHeadRef.current = { x: 0, y: 0 };
          stateRef.current.mouthOpen = REST_MOUTH;
          expressionControllerRef.current?.apply("neutral");
          break;
      }
    },

    applyExpression(expressionName) {
      expressionControllerRef.current?.apply(expressionName);
    },
  ...
  useEffect(() => {
    ...
    const initAvatar = async () => {
      try {
        ...
        expressionControllerRef.current = new ExpressionController(viewer);
...
export const AVATAR_REST_MOUTH = REST_MOUTH;